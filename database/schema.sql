-- Bookstore schema from ER diagram (MySQL 8.0.16+ / InnoDB)
--
-- Deploy with MySQL Workbench:
--   1) Database > Manage Connections > add Standard (TCP/IP): Host = your LAMP server IP, Port 3306, User/Password.
--   2) File > Open SQL Script > this file, then lightning-bolt Execute (or Server > Data Import > Run SQL Script).
-- Server must allow remote MySQL (bind-address, firewall port 3306, user GRANT for your client IP).
--
-- PHP on the LAMP box uses DB_HOST / DB_NAME / DB_USER / DB_PASSWORD (see includes/bootstrap.php); default DB name: bookstore.
