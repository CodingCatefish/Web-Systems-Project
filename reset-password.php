...
if ($method === 'GET') {
    ...
    if ($queryToken !== '') {
        $queryTokenStatus = password_reset_status($queryToken);
        if ($queryTokenStatus !== '') {
            clear_active_password_reset_token();
            redirect_with_query('reset-password.php', ['auth_error' => $queryTokenStatus]);
        }

        set_active_password_reset_token($queryToken);
        redirect_to('reset-password.php', 303);
    }
}

$token = active_password_reset_token();
...
if ($method === 'POST') {
    try {
        ...
        require_valid_form_post('reset-password.php');

        $validationError = validate_password_reset_input($password, $confirmPassword);
        if ($validationError !== '') {
            redirect_with_query('reset-password.php', ['auth_error' => $validationError]);
        }

        $resetError = reset_password_with_token($token, $password);
        if ($resetError !== '') {
            if ($resetError !== 'server_error') {
                clear_active_password_reset_token();
            }
            redirect_with_query('reset-password.php', ['auth_error' => $resetError]);
        }
